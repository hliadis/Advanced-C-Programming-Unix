...
void error_Sigaction(char * nameOfFunction, int error){
	fprintf(stderr, "ERROR: Sigaction failed(%s): %s.\n", nameOfFunction, strerror(error));
	fprintf(stderr, "Exiting program ....\n");
	exit(EXIT_FAILURE);
}

void error_Sigemptyset(char * nameOfFunction, int error){
	fprintf(stderr, "ERROR: Sigemptyset failed(%s): %s.\n", nameOfFunction, strerror(error));
	fprintf(stderr, "Exiting program ....\n");
	exit(EXIT_FAILURE);
}

void error_Sigaddset(char * nameOfFunction, int error){
	fprintf(stderr, "ERROR: Sigaddset failed(%s): %s.\n", nameOfFunction, strerror(error));
	fprintf(stderr, "Exiting program ....\n");
	exit(EXIT_FAILURE);
}

void error_Sigprocmask(char * nameOfFunction, int error){
	fprintf(stderr, "ERROR: Sigpromask failed(%s): %s.\n", nameOfFunction, strerror(error));
	fprintf(stderr, "Exiting program ....\n");
	exit(EXIT_FAILURE);
}

void error_Kill(char * nameOfFunction, int error){
	fprintf(stderr, "ERROR: Unable to send signal(%s): %s.\n", nameOfFunction, strerror(error));
	fprintf(stderr, "Exiting program ....\n");
	exit(EXIT_FAILURE);
}

void error_Setitimer(char * nameOfFunction, int error){
	fprintf(stderr, "ERROR: Unable to set timer(%s): %s.\n", nameOfFunction, strerror(error));
	fprintf(stderr, "Exiting program ....\n");
	exit(EXIT_FAILURE);
}

void error_Sigdelset(char * nameOfFunction, int error){
	fprintf(stderr, "ERROR: Sigdelset failed(%s): %s.\n", nameOfFunction, strerror(error));
	fprintf(stderr, "Exiting program ....\n");
	exit(EXIT_FAILURE);
}